...
export function pawnMove(
  ...
  if (to === enPassantSquare) {
    const enPassantRank =
      turn === "white"
        ? Number(enPassantSquare[1]) - 1
        : Number(enPassantSquare[1]) + 1;
    const enPassantRightFile = String.fromCharCode(
      enPassantSquare[0].charCodeAt(0) + 1,
    );
    const enPassantLeftFile = String.fromCharCode(
      enPassantSquare[0].charCodeAt(0) - 1,
    );
    if (
      fromRank === enPassantRank &&
      (fromFile === enPassantRightFile || fromFile === enPassantLeftFile)
    ) {
      return true;
    }
  }
