import { Action, State, Move } from "../../types";
  ...
  switch (action.type) {
    case "COMMIT_MOVE": {
      ...
      return {
        ...
        history: [...state.history, { ...result.move, board: result.board } as Move],
        redoStack: [],
      };
    }
    ...
    case "PROMOTE": {
      const promotedPiece = {
        ...
        value: PIECE_VALUE[action.piece],
      };
      ...
      if (!lastMove.piece) return state;
      const updatedMove = {
        ...lastMove,
        san: generateSAN(
          lastMove.from,
