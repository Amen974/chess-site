...
import { Difficulty, Mode, Piece, Side, State } from "../types";
...
export function useGameControls() {
  ...
  async function requestMove({ from, to, animate = false }: {from: string, to: string, animate: boolean}) {
    setViewingIndex(-1);
    ...
    if (result.turn === aiTurn) {
      startSpin();
      const aiResult = await playAIMove(result as unknown as State, aiDepth);
      if(!aiResult) return;
      stopSpin();
  ...
  const handleOnDrop = (to: string) => {
    if (isBrowsing) return;
    if (!dragFrom) return;
    requestMove({ from: dragFrom, to, animate: false });
  };
  ...
  const computeLegalMoves = (from: string) => {
    const moves = [];
  ...
  const handleStart = ({ mode, difficulty, side }: {mode: Mode, difficulty: Difficulty, side: Side}) => {
  ...
  const jumpToPosition = (i: number) => {
    setViewingIndex(i);
  };

  /* ================= AUTO SCROLL ================= */

  const sanRefMobile = useRef<HTMLDivElement>(null);
  const sanRefDesktop = useRef<HTMLDivElement>(null);
  ...
  const animateMove = (from: string, to: string, piece: Piece|null) => {
  ...
  const pieceBounce = (square: string) => {
